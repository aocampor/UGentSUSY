...
DPAnalysis::~DPAnalysis()
{
   ...
   hist->GetXaxis()->SetBinLabel(3,"Vertex");
   hist->GetXaxis()->SetBinLabel(4,"Photon");
   hist->GetXaxis()->SetBinLabel(5,"beamHalo");
   hist->GetXaxis()->SetBinLabel(6,"Jet");
...
bool DPAnalysis::EventSelection(const edm::Event& iEvent, const edm::EventSetup& iSetup ) {
   ...
   if ( passEvent && passTrigger ) hist->Fill(3);
   // Stupid PFIso 
   if ( passEvent ) { 
   ...
   if( beamHaloSummary.isValid() ) {
     ...
     if( !TheSummary.CSCTightHaloId() && passEvent && passTrigger) { 
       counter[4]++ ;
       hist->Fill(4);
     } else {
       passEvent = false ;
...
//    cout << "njets: "<< selectedJets.size()  << endl;
   if ( selectedJets.size() < jetCuts[3] )   passEvent = false ;
   if ( passEvent )   counter[5]++ ;
   if ( passEvent && passTrigger) hist->Fill(5);
   ...
   if ( passEvent && passTrigger) hist->Fill(6);
...
bool DPAnalysis::PhotonSelection( Handle<reco::PhotonCollection> photons, Handle<EcalRecHitCollection> recHitsEB, Handle<EcalRecHitCollection> recHitsEE, Handle<reco::TrackCollection> tracks, vector<const reco::Photon*>& selectedPhotons, const edm::Event& iEvent,const edm::EventSetup& iSetup ) {
  ...
  //END EXTRA DXY
   ...
   for(reco::PhotonCollection::const_iterator it = photons->begin(); it != photons->end(); it++) {
       ...
       reco::Photon aPho = reco::Photon(*it);
       reco::ConversionRefVector conversions = aPho.conversions();
       //cout << "Pho: " << aPho.et() << " ,K: " << k << endl;
       for (unsigned int iConv=0; iConv<conversions.size(); iConv++) {
	 reco::ConversionRef aConv=conversions[iConv];
	 double like = aConv->MVAout();
	 if ( like < 0.3 ) continue;
	 if (aConv->nTracks() != 2 ) continue;
	 if (!(aConv->conversionVertex().isValid())) continue;

	 const RefToBase<reco::Track> tk1 = aConv->tracks().front();
	 const RefToBase<reco::Track> tk2 = aConv->tracks().back();
	 if (tk1->numberOfValidHits()<5 || tk2->numberOfValidHits()<5) continue;

	 double chi2Prob = ChiSquaredProbability(aConv->conversionVertex().chi2(), aConv->conversionVertex().ndof());
	 if (chi2Prob<0.001)
	   continue;

	 GlobalPoint point(aConv->conversionVertex().position().x(),
			   aConv->conversionVertex().position().y(), aConv->conversionVertex().position().z());
	 ...
	 GlobalTrajectoryParameters theG = transTrajectoryParameters(point, posGtp);
	 GlobalTrajectoryParameters theH = transTrajectoryParameters(point, negGtp);

	 std::pair<GlobalVector,GlobalVector> momenta;
	 momenta.first = theG.momentum();
	 momenta.second = theH.momentum();

	 GlobalVector conv_photon_dir = momenta.first + momenta.second;

	 GlobalPoint point_wrt(point.x()-the_pvtx.position().x(), point.y()-the_pvtx.position().y(), point.z()-the_pvtx.position().z());


	 dxy = -point_wrt.x()*sin(conv_photon_dir.phi())+point_wrt.y()*cos(conv_photon_dir.phi());
	 dz = point_wrt.z()-((point_wrt.x()*conv_photon_dir.x()+point_wrt.y()*conv_photon_dir.y())/conv_photon_dir.perp())*(conv_photon_dir.z()/conv_photon_dir.perp());
