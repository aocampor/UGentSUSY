...
*/
	   ...
	   stack->GetXaxis()->SetTitle("BDT output");
          ...
          if (dir == "T2tt")    {dataset_name = "t2tt";}
           ...
           c2.Print("~/www/STOP/NNTraining/8TeV/Plots16/"+TString(dataset_name)+"/"+TString(setup)+"/"+TString(MVA)+"/histo_"+TString(stop)+"_"+TString(neut)+".png");
...
void makeCLsCards(TString setup, TString BDT, TString dir, int MSTOP, int MLSP){
	     ...
	     // cout << "2: " << ttbar_ << endl;
				...
				createTableCLs(setup, BDT, MSTOP, MLSP,  signal_, ttbar_, wjets_, others_);
...
void make2Dplot(TString dir, TString setup, TString BDT, int num){
          ...
          if (num ==3) {gStyle->SetPaintTextFormat("0.0f");}
	   ...
	   c1.Range(-289.7381,-191.8196,1334.643,1074.487);
           ...
           if (num==1) { ntpdir = "~/www/STOP/NNTraining/8TeV/Plots16/"+TString(dataset_name)+"/"+TString(setup)+"/"+TString(BDT)+"/FOM.png";}
           if (num==2) { ntpdir = "~/www/STOP/NNTraining/8TeV/Plots16/"+TString(dataset_name)+"/"+TString(setup)+"/"+TString(BDT)+"/BDT_cuts.png";}
           if (num==3) { ntpdir = "~/www/STOP/NNTraining/8TeV/Plots16/"+TString(dataset_name)+"/"+TString(setup)+"/"+TString(BDT)+"/BDT_Regions.png";}
...
tex->SetNDC();
   ...
   tex->Draw();
	  ...
	  TString ntpdir;
          ...
          if (num==1) { ntpdir = "~/www/STOP/NNTraining/8TeV/Plots16/"+TString(dataset_name)+"/"+TString(setup)+"/FOM.png";}
          if (num==2) { ntpdir = "~/www/STOP/NNTraining/8TeV/Plots16/"+TString(dataset_name)+"/"+TString(setup)+"/BDT_cuts.png";}
          if (num==3) { ntpdir = "~/www/STOP/NNTraining/8TeV/Plots16/"+TString(dataset_name)+"/"+TString(setup)+"/BDT_Regions.png";}
...
tex->SetNDC();
   ...
   tex->Draw();
	  ...
	  TString ntpdir;
          ...
          if (num==1) { ntpdir = "~/www/STOP/NNTraining/8TeV/Plots16/"+TString(dataset_name)+"_FOM.png";}
          if (num==2) { ntpdir = "~/www/STOP/NNTraining/8TeV/Plots16/"+TString(dataset_name)+"_BDT_cuts.png";}
          if (num==3) { ntpdir = "~/www/STOP/NNTraining/8TeV/Plots16/"+TString(dataset_name)+"_BDT_Regions.png";}


          if (num < 4) {c1.Print(ntpdir);}
...
//make2Dplot_MAX_OffShell("setup_0","setup_1","T2bw025",4);
//make2Dplot_MAX_OffShell("setup_0","setup_1","T2bw050",4);
//make2Dplot_MAX_OffShell("setup_0","setup_1","T2bw075",4);
//make2Dplot_MAX_OffShell("setup_2","setup_3","T2tt",4);
...
void createTableCLs(TString setup, TString BDT, int S, int N, double signal, double ttbar, double wjets, double others){

  char datacardname[100];
  sprintf(datacardname,"/afs/cern.ch/work/s/sigamani/public/CMSSW_6_1_1/src/HiggsAnalysis/CombinedLimit/datacard-S%d-N%d.txt",S,N);  
